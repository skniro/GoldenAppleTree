...
public class GoldenAppleTreeBlocks {
    ...
    public static final Block APPLE_SAPLING = registerBlock("apple_sapling",
             new SaplingBlock(AppleSaplingGenerator.AppleSapling, AbstractBlock.Settings.copyShallow(Blocks.OAK_SAPLING)));

    //LEAVES
    public static final Block Golden_APPLE_LEAVES =registerBlock("golden_apple_leave",
             new LeafCropBlock(AbstractBlock.Settings.create().nonOpaque().mapColor(MapColor.DARK_GREEN).strength(0.2F).sounds(BlockSoundGroup.GRASS).nonOpaque().suffocates(Blocks::never).blockVision(Blocks::never).burnable().pistonBehavior(PistonBehavior.DESTROY).solidBlock(Blocks::never), Items.GOLDEN_APPLE));
    public static final Block ENCHANTED_GOLDEN_APPLE_LEAVES =registerBlock("enchanted_golden_apple_leave",
             new LeafCropBlock(AbstractBlock.Settings.create().nonOpaque().mapColor(MapColor.DARK_GREEN).strength(0.2F).sounds(BlockSoundGroup.GRASS).nonOpaque().suffocates(Blocks::never).blockVision(Blocks::never).burnable().pistonBehavior(PistonBehavior.DESTROY).solidBlock(Blocks::never), Items.ENCHANTED_GOLDEN_APPLE));
    public static final Block APPLE_LEAVES =registerBlock("apple_leave",
            new LeafCropBlock(AbstractBlock.Settings.create().nonOpaque().mapColor(MapColor.DARK_GREEN).strength(0.2F).sounds(BlockSoundGroup.GRASS).nonOpaque().suffocates(Blocks::never).blockVision(Blocks::never).burnable().pistonBehavior(PistonBehavior.DESTROY).solidBlock(Blocks::never), Items.APPLE));
    public static final Block Apple_Tree_LEAVES =registerBlock("apple_tree_leave",
            new LeavesBlock(AbstractBlock.Settings.create().nonOpaque().mapColor(MapColor.DARK_GREEN).strength(0.2F).sounds(BlockSoundGroup.GRASS).nonOpaque().suffocates(Blocks::never).blockVision(Blocks::never).burnable().pistonBehavior(PistonBehavior.DESTROY).solidBlock(Blocks::never)));
    ...
    public static final Block POTTED_APPLE_SAPLING = registerBlockWithoutItem("potted_apple_sapling",
            new FlowerPotBlock(APPLE_SAPLING, AbstractBlock.Settings.create().breakInstantly().nonOpaque()));
