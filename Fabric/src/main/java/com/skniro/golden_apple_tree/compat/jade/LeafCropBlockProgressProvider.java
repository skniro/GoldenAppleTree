package com.skniro.golden_apple_tree.compat.jade;
...
public class LeafCropBlockProgressProvider implements IBlockComponentProvider {
    ...
    public static final Identifier GoldenAppleWaila = Identifier.of(GoldenAppleTree.MOD_ID, "apple_progress");

    @Override
    public @Nullable IElement getIcon(BlockAccessor accessor, IPluginConfig config, IElement currentIcon) {
        BlockState state = accessor.getBlockState();
        if (state.getBlock() instanceof LeafCropBlock leafCrop) {
            return IElementHelper.get().item(new ItemStack(leafCrop.fruitItem));
        }
        return currentIcon;
    }
